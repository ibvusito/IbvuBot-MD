...
await delay(2 * 2000)
await conn.sendMessage(res6, { text: text + '\n\n_atte. 𝐈𝐁𝐕𝐔 - 𝐁𝐎𝐓_', mentions: (await conn.groupMetadata(`${res6}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res7, { text: text + '\n\n_atte. 𝐈𝐁𝐕𝐔 - 𝐁𝐎𝐓_', mentions: (await conn.groupMetadata(`${res7}`)).participants.map(v => v.id) }, { quoted: fkontak })
await delay(2 * 2000)
await conn.sendMessage(res8, { text: text + '\n\n_atte. 𝐈𝐁𝐕𝐔 - 𝐁𝐎𝐓_', mentions: (await conn.groupMetadata(`${res8}`)).participants.map(v => v.id) }, { quoted: fkontak })
await conn.sendMessage(res9, { text: text + '\n\n_atte. 𝐈𝐁𝐕𝐔 - 𝐁𝐎𝐓_', mentions: (await conn.groupMetadata(`${res9}`)).participants.map(v => v.id) }, { quoted: fkontak })
